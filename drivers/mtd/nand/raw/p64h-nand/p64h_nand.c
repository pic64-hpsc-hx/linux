...
#define GCMD_LAY_CS GENMASK_ULL(10, 8)
/*
...
/* NVM SS interrupt register clear value */
#define NVM_SS_INT_CLEAR_VAL 0x40u
...
static inline struct cdns_nand_chip *to_cdns_nand_chip(struct nand_chip *chip)
{
	return container_of(chip, struct cdns_nand_chip, chip);
}

static inline struct cdns_nand_ctrl *
to_cdns_nand_ctrl(struct nand_controller *controller)
{
...
static int cadence_nand_wait_for_value(struct cdns_nand_ctrl *cdns_ctrl,
				       u32 reg_offset, u32 timeout_us, u32 mask,
				       bool is_clear)
{
	u32 val;
	int ret;

	ret = readl_relaxed_poll_timeout(cdns_ctrl->reg + reg_offset, val,
					 !(val & mask) == is_clear, 10,
					 timeout_us);
				       ...
				       bool enable)
{
	...
	if (cadence_nand_wait_for_value(cdns_ctrl, CTRL_STATUS, 1000000,
					    ...
					    u16 marker_value)
{
	...
	reg |= FIELD_PREP(SKIP_BYTES_MARKER_VALUE, marker_value);
...
static int cadence_nand_set_skip_bytes_conf(struct cdns_nand_ctrl *cdns_ctrl,
					    u8 num_of_bytes, u32 offset_value,
					    int enable)
{
	...
	reg |= FIELD_PREP(SKIP_BYTES_NUM_OF_BYTES, num_of_bytes);
	skip_bytes_offset = FIELD_PREP(SKIP_BYTES_OFFSET_VALUE, offset_value);
...
static void cadence_nand_set_erase_detection(struct cdns_nand_ctrl *cdns_ctrl,
					     bool enable, u8 bitflips_threshold)
{
	u32 reg;
...
static void
cadence_nand_clear_nvm_ss_interrupt(struct cdns_nand_ctrl *cdns_ctrl)
{
	writel_relaxed(NVM_SS_INT_CLEAR_VAL, cdns_ctrl->nvm_ctrl_i_reg);
}
			     ...
			     struct cadence_nand_irq_status *irq_status)
{
	...
	irq_status->trd_status =
		readl_relaxed(cdns_ctrl->reg + TRD_COMP_INT_STATUS);
	irq_status->trd_error =
		readl_relaxed(cdns_ctrl->reg + TRD_ERR_INT_STATUS);
}
...
static irqreturn_t cadence_nand_isr(int irq, void *dev_id)
{
	...
	if (irq_detected(cdns_ctrl, &irq_status)) {
		...
		if (cdns_ctrl->irq_status.status)
			complete(&cdns_ctrl->complete);
		/* Tell the OS that we've handled this. */
		result = IRQ_HANDLED;
	} else {
		cadence_nand_clear_nvm_ss_interrupt(cdns_ctrl);
		result = IRQ_HANDLED;
	}
...
static int cadence_nand_wait_on_sdma(struct cdns_nand_ctrl *cdns_ctrl,
				     u8 *out_sdma_trd, u32 *out_sdma_size)
{
	struct cadence_nand_irq_status irq_mask, irq_status;

	irq_mask.trd_status = 0;
	irq_mask.trd_error = 0;
	irq_mask.status = INTR_STATUS_SDMA_TRIGG | INTR_STATUS_SDMA_ERR |
	...
	if (irq_status.status & INTR_STATUS_SDMA_TRIGG) {
		...
		*out_sdma_trd = FIELD_GET(SDMA_TRD_NUM_SDMA_TRD, *out_sdma_trd);
	} else {
...
static void cadence_nand_get_caps(struct cdns_nand_ctrl *cdns_ctrl)
{
	...
	if (reg & (CTRL_FEATURES_NVDDR_2_3 | CTRL_FEATURES_NVDDR))
		cdns_ctrl->caps2.is_phy_type_dll = true;
}

// static void cadence_nand_cdma_desc_print(struct cadence_nand_cdma_desc *desc)
// {
//     printk("cdma_desc contents:\n");
//     printk("  next_pointer      : 0x%016llx\n",
//            (unsigned long long)desc->next_pointer);
//     printk("  flash_pointer     : 0x%08x\n", desc->flash_pointer);
//     printk("  bank              : 0x%04x\n", desc->bank);
//     printk("  rsvd0             : 0x%04x\n", desc->rsvd0);
//     printk("  command_type      : 0x%04x\n", desc->command_type);
//     printk("  rsvd1             : 0x%04x\n", desc->rsvd1);
//     printk("  command_flags     : 0x%04x\n", desc->command_flags);
//     printk("  rsvd2             : 0x%04x\n", desc->rsvd2);
//     printk("  memory_pointer    : 0x%016llx\n",
//            (unsigned long long)desc->memory_pointer);
//     printk("  status            : 0x%08x\n", desc->status);
//     printk("  rsvd3             : 0x%08x\n", desc->rsvd3);
//     printk("  sync_flag_pointer : 0x%016llx\n",
//            (unsigned long long)desc->sync_flag_pointer);
//     printk("  sync_arguments    : 0x%08x\n", desc->sync_arguments);
//     printk("  rsvd4             : 0x%08x\n", desc->rsvd4);
//     printk("  ctrl_data_ptr     : 0x%016llx\n",
//            (unsigned long long)desc->ctrl_data_ptr);
// }

/* Prepare CDMA descriptor. */
static void cadence_nand_cdma_desc_prepare(struct cdns_nand_ctrl *cdns_ctrl,
					   char nf_mem, u32 flash_ptr,
					   dma_addr_t mem_ptr,
					   dma_addr_t ctrl_data_ptr, u16 ctype)
{
	...
	// cadence_nand_cdma_desc_print(cdma_desc);
}
...
static int cadence_nand_cdma_send(struct cdns_nand_ctrl *cdns_ctrl, u8 thread)
{
	u32 reg;
	int status;

	/* Wait for thread ready. */
	status = cadence_nand_wait_for_value(cdns_ctrl, TRD_STATUS, 1000000,
					     BIT(thread), true);
	if (status)
		return status;
	...
	writel_relaxed((u32)(cdns_ctrl->dma_cdma_desc >> 32),
		       cdns_ctrl->reg + CMD_REG3);
...
#define CADENCE_NAND_CALC_ECC_BYTES(max_step_size)                            \
	static int cadence_nand_calc_ecc_bytes_##max_step_size(int step_size, \
							       int strength)  \
	{                                                                     \
...
static int cadence_nand_read_bch_caps(struct cdns_nand_ctrl *cdns_ctrl)
{
	...
	cdns_ctrl->ecc_stepinfos[0].stepsize = FIELD_GET(BCH_CFG_2_SECT_0, reg);

	cdns_ctrl->ecc_stepinfos[1].stepsize = FIELD_GET(BCH_CFG_2_SECT_1, reg);
...
static int cadence_nand_hw_init(struct cdns_nand_ctrl *cdns_ctrl)
{
	...
	status = cadence_nand_wait_for_value(cdns_ctrl, CTRL_STATUS, 1000000,
	...
	dev_info(cdns_ctrl->dev, "%s: cadence nand controller version reg %x\n",
		 __func__, reg);
					   ...
					   int transfer_type)
{
	...
	case TT_MAIN_OOB_AREAS:
		sec_cnt = cdns_chip->sector_count;
		last_sec_size =
			cdns_chip->sector_size + cdns_chip->avail_oob_size;
		sec_size = cdns_chip->sector_size;
		break;
	case TT_RAW_PAGE:
				      ...
				      bool with_ecc)
{
	...
	// Force 64bits mask
	// TODO: check why mask is 32bits instead of 64bits
	status = dma_set_mask_and_coherent(cdns_ctrl->dev, DMA_BIT_MASK(64));
	if (status) {
		BUG();
	}
	...
	if (ctrl_dat && ctrl_dat_size) {
		...
		if (dma_mapping_error(cdns_ctrl->dev, dma_ctrl_dat)) {
			dma_unmap_single(cdns_ctrl->dev, dma_buf, buf_size,
					 dir);
			dev_err(cdns_ctrl->dev, "Failed to map DMA buffer\n");
			return -EIO;
		}
	...
	cadence_nand_cdma_desc_prepare(cdns_ctrl, chip_nr, page, dma_buf,
				       dma_ctrl_dat, ctype);

	status = cadence_nand_cdma_send_and_wait(cdns_ctrl, thread_nr);

	dma_unmap_single(cdns_ctrl->dev, dma_buf, buf_size, dir);

	if (ctrl_dat && ctrl_dat_size)
		dma_unmap_single(cdns_ctrl->dev, dma_ctrl_dat, ctrl_dat_size,
				 dir);
	if (status)
		return status;
...
static int cadence_nand_select_target(struct nand_chip *chip)
{
	...
	cdns_ctrl->curr_corr_str_idx = 0xf; /* Invalid index to force update of ECC strength. */
	cadence_nand_set_ecc_strength(cdns_ctrl, cdns_chip->corr_str_idx);

	cadence_nand_set_erase_detection(cdns_ctrl, true, chip->ecc.strength);
...
static int cadence_nand_read_bbm(struct nand_chip *chip, int page, u8 *buf)
{
	...
	status = cadence_nand_cdma_transfer(cdns_ctrl,
					    cdns_chip->cs[chip->cur_cs], page,
					    cdns_ctrl->buf, NULL, mtd->oobsize,
...
static int cadence_nand_write_page(struct nand_chip *chip, const u8 *buf,
				   int oob_required, int page)
{
	...
	cadence_nand_set_skip_bytes_conf(cdns_ctrl, cdns_chip->bbm_len,
					 mtd->writesize + cdns_chip->bbm_offs,
					 1);

	if (oob_required) {
		marker_val = *(u16 *)(chip->oob_poi + cdns_chip->bbm_offs);
	} else {
	...
	if (cadence_nand_dma_buf_ok(cdns_ctrl, buf, mtd->writesize) &&
		...
		status = cadence_nand_cdma_transfer(
			cdns_ctrl, cdns_chip->cs[chip->cur_cs], page,
			(void *)buf, oob, mtd->writesize,
			cdns_chip->avail_oob_size, DMA_TO_DEVICE, true);
		if (status) {
	...
	return cadence_nand_cdma_transfer(
		cdns_ctrl, cdns_chip->cs[chip->cur_cs], page, cdns_ctrl->buf,
		NULL, mtd->writesize + cdns_chip->avail_oob_size, 0,
		DMA_TO_DEVICE, true);
}
...
static int cadence_nand_write_page_raw(struct nand_chip *chip, const u8 *buf,
				       int oob_required, int page)
{
	struct cdns_nand_ctrl *cdns_ctrl = to_cdns_nand_ctrl(chip->controller);
	struct cdns_nand_chip *cdns_chip = to_cdns_nand_chip(chip);
	...
	return cadence_nand_cdma_transfer(
		cdns_ctrl, cdns_chip->cs[chip->cur_cs], page, cdns_ctrl->buf,
		NULL, mtd->writesize + mtd->oobsize, 0, DMA_TO_DEVICE, false);
}

static int cadence_nand_write_oob_raw(struct nand_chip *chip, int page)
{
	return cadence_nand_write_page_raw(chip, NULL, true, page);
}

static int cadence_nand_read_page(struct nand_chip *chip, u8 *buf,
				  int oob_required, int page)
{
	struct cdns_nand_ctrl *cdns_ctrl = to_cdns_nand_ctrl(chip->controller);
	struct cdns_nand_chip *cdns_chip = to_cdns_nand_chip(chip);
	...
	if (cadence_nand_dma_buf_ok(cdns_ctrl, buf, mtd->writesize) &&
		...
		status = cadence_nand_cdma_transfer(
			cdns_ctrl, cdns_chip->cs[chip->cur_cs], page, buf, oob,
			mtd->writesize, cdns_chip->avail_oob_size,
		...
		status = cadence_nand_cdma_transfer(
			cdns_ctrl, cdns_chip->cs[chip->cur_cs], page,
			cdns_ctrl->buf, NULL,
			mtd->writesize + cdns_chip->avail_oob_size, 0,
			DMA_FROM_DEVICE, true);

		memcpy(buf, cdns_ctrl->buf, mtd->writesize);
		if (oob_required)
			memcpy(chip->oob_poi, cdns_ctrl->buf + mtd->writesize,
			       mtd->oobsize);
	}
	...
	case STAT_ECC_CORR:
		ecc_err_count =
			FIELD_GET(CDMA_CS_MAXERR, cdns_ctrl->cdma_desc->status);
...
static int cadence_nand_read_page_raw(struct nand_chip *chip, u8 *buf,
				      int oob_required, int page)
{
	struct cdns_nand_ctrl *cdns_ctrl = to_cdns_nand_ctrl(chip->controller);
	struct cdns_nand_chip *cdns_chip = to_cdns_nand_chip(chip);
	...
	status = cadence_nand_cdma_transfer(
		cdns_ctrl, cdns_chip->cs[chip->cur_cs], page, cdns_ctrl->buf,
		NULL, mtd->writesize + mtd->oobsize, 0, DMA_FROM_DEVICE, false);
...
static int cadence_nand_read_oob_raw(struct nand_chip *chip, int page)
{
...
static int cadence_nand_read_buf(struct cdns_nand_ctrl *cdns_ctrl, u8 *buf,
				 int len)
{
	u8 thread_nr = 0;
	u32 sdma_size;
	...
	if (!cdns_ctrl->caps1->has_dma) {
		u8 data_dma_width = cdns_ctrl->caps2.data_dma_width;
		u32 len_in_words = (data_dma_width == 4) ? ((len + 3) >> 2) :
							   ((len + 7) >> 3);

		// Use a temporary buffer large enough for the aligned read
		u8 tmp_buf[8]; // Maximum possible word size is 8 bytes (64-bit)

		if (len < data_dma_width) {
			// Read one word into tmp_buf, then copy only 'len' bytes to buf
			if (data_dma_width == 4)
				ioread32_rep(cdns_ctrl->io.virt, tmp_buf, 1);
#ifdef CONFIG_64BIT
			else
				readsq(cdns_ctrl->io.virt, tmp_buf, 1);
#endif
			memcpy(buf, tmp_buf, len);
		} else {
			// Read as many full words as possible directly into buf
			if (data_dma_width == 4)
				ioread32_rep(cdns_ctrl->io.virt, buf,
					     len_in_words);
#ifdef CONFIG_64BIT
			else
				   ...
				   const struct nand_subop *subop)
{
	...
	mini_ctrl_cmd |= FIELD_PREP(GCMD_LAY_INPUT_CMD, instr->ctx.cmd.opcode);

	ret = cadence_nand_generic_cmd_send(
		cdns_ctrl, cdns_chip->cs[chip->cur_cs], mini_ctrl_cmd);
	if (ret)
				    ...
				    const struct nand_subop *subop)
{
	...
	mini_ctrl_cmd |= FIELD_PREP(GCMD_LAY_INSTR, GCMD_LAY_INSTR_ADDR);
	...
	mini_ctrl_cmd |= FIELD_PREP(GCMD_LAY_INPUT_ADDR, address);
	mini_ctrl_cmd |= FIELD_PREP(GCMD_LAY_INPUT_ADDR_SIZE, naddrs - 1);
				 ...
				 const struct nand_subop *subop)
{
	...
	mini_ctrl_cmd |= FIELD_PREP(GCMD_LAY_INSTR, GCMD_LAY_INSTR_DATA);
				    ...
				    const struct nand_subop *subop)
{
	...
	status = cadence_nand_wait_for_value(cdns_ctrl, RBN_SETINGS, timeout_us,
...
static const struct nand_op_parser cadence_nand_op_parser = NAND_OP_PARSER(
	NAND_OP_PARSER_PATTERN(
		cadence_nand_cmd_erase, NAND_OP_PARSER_PAT_CMD_ELEM(false),
		NAND_OP_PARSER_PAT_ADDR_ELEM(false, MAX_ERASE_ADDRESS_CYC),
		NAND_OP_PARSER_PAT_CMD_ELEM(false),
		NAND_OP_PARSER_PAT_WAITRDY_ELEM(false)),
	NAND_OP_PARSER_PATTERN(cadence_nand_cmd_opcode,
			       NAND_OP_PARSER_PAT_CMD_ELEM(false)),
	NAND_OP_PARSER_PATTERN(cadence_nand_cmd_address,
			       NAND_OP_PARSER_PAT_ADDR_ELEM(false,
							    MAX_ADDRESS_CYC)),
	NAND_OP_PARSER_PATTERN(cadence_nand_cmd_data,
			       NAND_OP_PARSER_PAT_DATA_IN_ELEM(false,
							       MAX_DATA_SIZE)),
	NAND_OP_PARSER_PATTERN(cadence_nand_cmd_data,
			       NAND_OP_PARSER_PAT_DATA_OUT_ELEM(false,
								MAX_DATA_SIZE)),
	NAND_OP_PARSER_PATTERN(cadence_nand_cmd_waitrdy,
			       NAND_OP_PARSER_PAT_WAITRDY_ELEM(false)));
				...
				u32 board_delay_skew_min, u32 ext_mode)
{
	...
	return (trp_cnt + 1) * clk_period + trhoh_min + board_delay_skew_min;
}
			     ...
			     const struct nand_interface_config *conf)
{
	...
	u32 clk_period =
		DIV_ROUND_DOWN_ULL(1000000000000ULL, cdns_ctrl->nf_clk_rate);
	 ...
	 * mode 5.
	 */
	if (sdr->tRC_min <= clk_period && sdr->tRP_min <= (clk_period / 2) &&
		...
		if (tdvw > 0) {
			...
			if ((((tdvw_max / dqs_sampl_res) * dqs_sampl_res) <=
			     tdvw_min) ||
			    (((tdvw_max % dqs_sampl_res) == 0) &&
	...
	if (sdr->tWC_min <= clk_period &&
		...
		if ((twp_cnt + 1) * clk_period < (sdr->tALS_min + if_skew))
			twp_cnt =
				calc_cycl(sdr->tALS_min + if_skew, clk_period);
    ...
    if(reg != 0x9090909) {
        t->async_toggle_timings = 0x9090909;
        t->timings0 = 0x4f631727;
        t->timings1 = 0x28300063;
        t->timings2 = 0xc7030d;
        t->dll_phy_ctrl = 0x01030707;
		t->phy_ctrl = 0x000040a0;
        t->phy_dqs_timing = 0x110004;
        t->phy_gate_lpbk_ctrl = 0x680000;
		dev_info(cdns_ctrl->dev, "Using default timings\n");
        return 0;
    }
				  ...
				  struct device_node *np)
{
	...
	cdns_chip = devm_kzalloc(cdns_ctrl->dev,
				 sizeof(*cdns_chip) + (nsels * sizeof(u8)),
				 GFP_KERNEL);
	if (!cdns_chip) {
	...
	for (i = 0; i < nsels; i++) {
		...
		if (cs >= cdns_ctrl->caps2.max_banks) {
			dev_err(cdns_ctrl->dev,
				...
				cdns_ctrl->caps2.max_banks);
			return -EINVAL;
		}

		if (test_and_set_bit(cs, &cdns_ctrl->assigned_cs)) {
			dev_err(cdns_ctrl->dev, "CS %d already assigned\n", cs);
			return -EINVAL;
		}
	...
	if (ret) {
		dev_err(cdns_ctrl->dev, "failed to register mtd device: %d\n",
			ret);
		nand_cleanup(chip);
		return ret;
	}
...
static int cadence_nand_init(struct cdns_nand_ctrl *cdns_ctrl)
{
	...
	printk("successfully loaded p64h-nand\n");
	return 0;
...
static const struct of_device_id cadence_nand_dt_ids[] = {
	{ .compatible = "microchip,p64h-nand", .data = &cadence_nand_default },
	{}
};
...
static int cadence_nand_dt_probe(struct platform_device *ofdev)
{
	...
	cdns_ctrl->io.virt =
		devm_platform_get_and_ioremap_resource(ofdev, 1, &res);
	if (IS_ERR(cdns_ctrl->io.virt))
		return PTR_ERR(cdns_ctrl->io.virt);
	cdns_ctrl->io.dma = res->start;

	cdns_ctrl->nvm_ctrl_i_reg = devm_platform_ioremap_resource(ofdev, 2);
	if (IS_ERR(cdns_ctrl->nvm_ctrl_i_reg))
		return PTR_ERR(cdns_ctrl->nvm_ctrl_i_reg);
	...
	ret = of_property_read_u32(ofdev->dev.of_node, "cdns,board-delay-ps",
				   &val);
	if (ret) {
		val = 4830;
		dev_info(cdns_ctrl->dev,
	...
	ret = dma_set_mask_and_coherent(cdns_ctrl->dev, DMA_BIT_MASK(64));
	if (ret) {
		dev_err(cdns_ctrl->dev, "No suitable DMA64 configuration\n");
		ret = dma_set_mask_and_coherent(cdns_ctrl->dev,
						DMA_BIT_MASK(32));
		if (ret) {
			dev_err(cdns_ctrl->dev,
				"No suitable DMA32 configuration\n");
			return ret;
		}
	}
